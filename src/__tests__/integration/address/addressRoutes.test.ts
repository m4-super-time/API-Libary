...
describe("/address", () => {
  ...
  test("POST /address/:id - Must be able to create a address", async () => {
    ...
    const response = await request(app)
      .post(`/address/${userLoginResponse.body.user.id}`)
      .set("Authorization", `Bearer ${userLoginResponse.body.token}`)
      .send(mockedUserAddress);
  ...
  test("GET /address - Must be able to list all address", async () => {
    ...
    await request(app)
      .post(`/address/${employeeLoginResponse.body.user.id}`)
      .set("Authorization", `Bearer ${employeeLoginResponse.body.token}`)
      .send(mockedEmployeeAddress);
    const response = await request(app)
      .get("/address")
      .set("Authorization", `Bearer ${employeeLoginResponse.body.token}`);
  ...
  test("GET /address/:id - Should be able to user get a specific address", async () => {
    const userLoginResponse = await request(app)
      ...
      .send(mockedUserLogin);
    const response = await request(app)
      .get(`/address/${userLoginResponse.body.user.id}`)
      .set("Authorization", `Bearer ${userLoginResponse.body.token}`);
  ...
  test("PATCH /address/:id - Should not be able to update a address without authentication", async () => {
    ...
    const userLoginResponse = await request(app)
      .post("/login")
      .send(mockedUserLogin);
    const addressToBeUpdate = await request(app).get(
      `/address/${userLoginResponse.body.id}`
    );

    const response = await request(app)
      .patch(`/address/${addressToBeUpdate.body.id}`)
      .send(newValue);
  ...
  test("PATCH /address/:id - Should not be able to update address with invalid id", async () => {
    ...
    const response = await request(app)
      .patch(`/address/13970660-5dbe-423a-9a9d-5c23b37943cf`)
      .set("Authorization", `Bearer ${userLoginResponse.body.token}`)
      .send(newValue);
  ...
  test("PATCH /address/:id - Should not be able to update address id field", async () => {
    ...
    const userLoginResponse = await request(app)
      .post("/login")
      .send(mockedUserLogin);
    const addressToBeUpdate = await request(app)
      .get("/address")
      .set("Authorization", `Bearer ${employeeLoginResponse.body.token}`);

    const response = await request(app)
      .patch(`/address/${addressToBeUpdate.body[0].id}`)
      .set("Authorization", `Bearer ${userLoginResponse.body.token}`)
      .send(newValue);
  ...
  test("DELETE /address/delete/:id - Should not be able to delete address without authentication", async () => {
    const userLoginResponse = await request(app)
      .post("/login")
      .send(mockedUserLogin);
    const addressToBeDeleted = await request(app)
      .get(`/address/${userLoginResponse.body.user.id}`)
      .set("Authorization", `Bearer ${userLoginResponse.body.token}`);

    const response = await request(app).delete(
      `/address/delete/${addressToBeDeleted.body.id}`
    );
  ...
  test("DELETE /address/delete/:id - Should not be able to delete another user address not being employee", async () => {
    ...
    const addressToBeDeleted = await request(app)
      .get(`/address/${employeeLoginResponse.body.user.id}`)
      .set("Authorization", `Bearer ${userLoginResponse.body.token}`);

    const response = await request(app)
      .delete(`/address/delete/${addressToBeDeleted.body.id}`)
      .set("Authorization", `Bearer ${userLoginResponse.body.token}`);
  ...
  test("DELETE /address/delete/:id - Must be able to delete address", async () => {
    ...
    const response = await request(app)
      .delete(`/address/delete/${userLoginResponse.body.user.id}`)
      .set("Authorization", `Bearer ${userLoginResponse.body.token}`);
    ...
    const addressList = await request(app)
      .get("/address")
      .set("Authorization", `Bearer ${employeeLoginResponse.body.token}`);
