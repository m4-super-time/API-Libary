...
export const listAllAddAdressesEmployeeControllers = async (
  req: Request,
  res: Response
) => {
  const listAllWithEmployeePermission =
    await listAllAddAdressesEmployeeService();
...
export const addressLookupByUserIdController = async (
  ...
  const searchAddressByUserId = await addressLookupByUserIdService(
    req.params.id
  );
